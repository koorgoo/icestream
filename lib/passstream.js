...
PassStream.prototype._processPassers = function(chunk) {
  for (var i = 0; i < this._passers.length; i++) {
    ...
    this._chunkers.unshift({
      size: delta,
      chunk: chunk.slice(0, delta),
      transform: this._noop
    });

    ps.size -= chunk.length;
...
PassStream.prototype._processChunkers = function(chunk) {
  for (var i = 0; i < this._chunkers.length; i++) {
    ...
    var offset = ch.chunk.length - ch.size;

    var slice = chunk.slice(0, toRead);
    ch.chunk.write(chunk.slice(0, toRead).toString(), offset);
    ch.size -= toRead;

    if (ch.size === 0) ch.remove = true;

    var size = chunk.length - toRead;
    var orig = chunk;
    chunk = new Buffer(size);

    chunk.write(orig.slice(toRead).toString());
  }
